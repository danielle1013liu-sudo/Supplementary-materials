...
def create_advanced_features(X):
    ...
    if len(numeric_cols) >= 2:
        # Interaction features inspired by TNF-α correlation analysis
...
def safe_mape(y_true, y_pred, eps=1e-8):
    """MAPE with safeguard for zero denominators, returns percentage"""
...
def main():
    print("TNF-α Random Forest Final Model - Log Transformation + Grid Search")
    ...
    y = np.log1p(df['TNF-α'])  # Log transform
    ...
    print(f"\n{'='*50}")
    print("Final TNF-α Random Forest Model Results")
    print(f"{'='*50}")
    ...
    # Save results
    results_df.to_csv('tnfa_final_results_rf.csv', index=False)
    print("\nResults saved to tnfa_final_results_rf.csv")
